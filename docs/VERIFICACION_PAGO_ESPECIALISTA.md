...
## 7. Verificación en tres capas (no procesar si no se ha pagado)

Para que una solicitud no se procese sin pago, este repo aplica verificaciones en **blockchain**, **API (release-now)** y **frontend**. El backend debe complementar con **BD**.

### 7.1 Blockchain

- **confirm-payment** (`POST /api/consultations/confirm-payment`): antes de llamar al backend, se **verifica la transacción en NEAR** con `verifyNearTransaction(txHash, relayerAccountId)`. Si la tx no existe o falló, se responde 400 y no se actualiza la consulta. Así no se puede confirmar un pago con un `txHash` falso.

### 7.2 API / release-now

- **release-now** (`POST /api/consultations/release-now`): antes de transferir USDT, se hace **GET** al backend `GET /api/consultations/:id`. Si el backend responde 200, se exige que la consulta esté pagada (`paid_at`, `paid_at`, `amount_raw` o `paymentStatus === 'paid'`). Si no está pagada, se responde 400 y no se libera. Si el monto en el body no coincide con `amount_raw` de la consulta, también 400. Si el backend no tiene GET por id (404), se registra un warning y se continúa para no romper integraciones antiguas.

### 7.3 Frontend

- En la pestaña **Especialista** de `/analisis`, el botón **Dictaminar** solo se muestra si la consulta está **pagada**: se considera pagada si la consulta tiene `paidAt`, `paid_at`, `amount_raw` o `paymentStatus === 'paid'/'Paid'`. Si no, se muestra el mensaje *"Esperando confirmación de pago"* y no se puede abrir el modal de dictamen.

### 7.4 Backend (BD) – lo que debe implementar

Para cerrar el flujo por completo, el backend debe:

1. **GET `/api/consultations/:id`**: devolver la consulta con al menos `paid_at` (o `paidAt`), `amount_raw` (o `amountRaw`) y/o `paymentStatus`, para que release-now pueda comprobar que está pagada.
2. **Listas de consultas** (p. ej. `GET /api/consultations/specialist/:account`): incluir en cada ítem `paid_at`/`paidAt`, `amount_raw`/`amountRaw` o `paymentStatus`, para que el frontend pueda mostrar u ocultar "Dictaminar".
3. **PATCH `/api/consultations/:id`** (cambio a `status: "attended"`): **rechazar** la actualización si la consulta no está pagada (p. ej. no tiene `paid_at` o `amount_raw`). Así, aunque alguien bypasee el frontend, el backend no guardará "attended" sin pago.
4. **POST `/api/consultations/:id/confirm-payment`**: guardar `paid_at`, `amount_raw`, `escrow_tx_hash` (o equivalente) al confirmar el pago (este repo ya verifica la tx on-chain antes de llamar a este endpoint).
